...
import { NetworkType, MetadataHttp } from 'symbol-sdk';
import * as CryptoJS from 'crypto-js';

const network = NetworkType.TEST_NET;
const filename = "Jeff's favorite car.pdf";
const file_content = CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse('New content !'));
...
metadataHttp.getAccountMetadata(apostille.account.address).toPromise().then((metadata) => {
    ...
    console.log("Could not update Apostille, no metadata found for the dedicated account");
});
